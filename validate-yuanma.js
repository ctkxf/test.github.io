...
<form id="testForm" enctype="multipart/form-data" class="form-horizontal">  
  <div class="form-group">  
    ...
    <div class="col-sm-5">  
      <input type="text" class="form-control" id="name" placeholder="请填写姓名" maxlength="50" name="name">  
    </div>          
  </div>  
  <div class="btn-wrapper">  
    <button class="btn btn-primary" type="submit" id="btnSave" data-complete-text="Loading..." autocomplete="off">提交</button>  
    <button class="btn btn-default" type="button" id="btnCleanForm">重置</button>  
    <button class="btn btn-default" type="button" id="btnReturn">返回</button>  
  </div>  
</form>  
...
$("#testForm").validate({  
  ...
  highlight: function( element, errorClass, validClass ) {  
    ...
    $(element).siblings('.glyphicon-ok').remove();  
    $('<span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>').insertBefore(element);  
  },  
  // 验证成功时添加成功提示文案  
  unhighlight: function ( element, errorClass, validClass ) {  
    ...
    if ( $(element).parent().parent().hasClass("has-success") ){  
      return;  
    };  
    $(element).siblings('.glyphicon-remove').remove();  
    $(element).parent().parent().removeClass("has-error").addClass("has-success").addClass("has-feedback");  
    $('<span class="glyphicon glyphicon-ok form-control-feedback" aria-hidden="true"></span>').insertBefore(element);  
  },  
  // 修改提交方式，返回return true时原始表单提交，无返回或返回否值改为另一种方案  
  submitHandler:function(form){  
    $.ajax({  
      name:$("#name").val();  
    })  
  },  
  // 调整错误提示的位置，checkbox、radio情况下移动到祖先元素内部  
